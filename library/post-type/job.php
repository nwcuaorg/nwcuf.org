...
function job_list() {
	...
	// select jobs and sort them by the job title.
	$args = array(
		'post_type' => 'job',
		'orderby' => 'post_name',
		'order' => 'ASC'
	...
	// generate the code for the job list
	$code = '<div class="job-list">';

	// if we have results
	if ( !empty( $jobs ) ) {
		foreach ( $jobs as $job ) {
			$code .= '<div class="job-entry">';
			$code .= '<h4><a href="/job/' . $job->post_name . '">' . $job->post_title . '</a></h4>';
			$code .= '<p>' . wp_trim_words( $job->post_content, 40 ) . '</p>';
			$code .= '</div>';
		}
	} else {
		// if no results, just return a little note
		$code .= '<p>No volunteer opportunities found.</p>';
	}
...
add_shortcode( 'job-list', 'job_list' );
